...
class MaskedLinear(torch.nn.Linear):
    ...
    def __init__(self, input_size, n_outputs, mask, cond_label_size=None):
        ...
        if cond_label_size is not None:
            self.cond_weight = torch.nn.Parameter(torch.rand(n_outputs, cond_label_size) / math.sqrt(cond_label_size))

    def forward(self, x, y=None):
        out = torch.nn.functional.linear(x, self.weight * self.mask, self.bias)
        if y is not None:
            out = out + torch.nn.functional.linear(y, self.cond_weight)
        return out
...
class BatchNorm(torch.nn.Module):
    ...
    def __init__(self, input_size, momentum=0.9, eps=1e-5):
        ...
        self.log_gamma = torch.nn.Parameter(torch.zeros(input_size))
...
class FlowSequential(torch.nn.Sequential):
